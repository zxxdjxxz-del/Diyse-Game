# Diyse — Seyrik Native Legacies
**Migration baseline:** `Diyse_CURRENT_WORKING_TRACKER_CONSOLIDATED_2026-08-27_v85.md`  
**Primary written equipment authority:** compatible **Audit117 / Audit118 / Audit121**, plus newer accepted v85 tracker-level equipment closures.  
**Current written whole-project authority:** **v2.20 / Audit135**.  
**Rule:** where v85 contains a later explicitly accepted equipment decision that has not yet been promoted, preserve it as **tracker-level final / pending formal promotion** rather than rewriting older audit history.


**Status:** tracker-level final / pending formal master-canon promotion.

| Legacy | Slot | Raw stats | Capstone | Trait |
|---|---|---|---|---|
| **You Are Finished.** | Two-Handed Sword — Weapon + Secondary | +105 ATK / +7 MAG | +10pp Critical Chance | **End It** |
| **You Should Have Killed Me.** | Battle Heavy Armor | +56 DEF / +34 SPR | Max HP +15% | **Still Dangerous** |


## You Are Finished. — End It
Against an enemy currently suffering at least one eligible:
- harmful status; or
- ordinary negative stat change;

eligible damaging Two-Handed Sword actions gain:
- **+15% final damage**
- **+15pp Defense penetration**.

Capstone:
- **+10pp Critical Chance**.

No instant-kill or boss-bypass behavior.

## You Should Have Killed Me. — Still Dangerous
After eligible hostile direct damage greater than 0 is dealt to wearer:
through end of following round gain:
- **+15% Attack**
- **+15% Magic**
- **+10pp harmful-status application chance** for already-authored harmful-status riders on wearer's eligible actions.

Repeated qualifying damage refreshes duration; magnitude does not stack.
No minimum damage threshold.
No automatic Bleed, heal, mitigation, counter, free action, or revenge gauge.

Capstone:
- **Max HP +15%**.


## Donor legality
The Legacy Trait travels with the actual item and must function for any legal donor receiver.
No universal off-owner penalty exists.

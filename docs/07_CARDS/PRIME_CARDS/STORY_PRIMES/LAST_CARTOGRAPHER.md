# Diyse — Prime: Last Cartographer
**Migration baseline:** `Diyse_CURRENT_WORKING_TRACKER_CONSOLIDATED_2026-08-27_v85.md`  
**Primary Card/Prime authority:** compatible **Audit116**, superseded where applicable by **Audit119**, **Audit122**, and later current v85 working closures.  
**Current written whole-project authority:** **v2.20 / Audit135**.  


**Face:** Acuity  
**Narrative bearer:** Torren  
**Progression:** Recovered → Awakened

## Command package
| Command | Target | Formula | Power | Base Hit | Effect |
|---|---|---|---:|---:|---|
| **Recovered — True Bearing** | one enemy | Physical / Neutral | 320 | 125 | 35% Defense penetration; 35% Stun; reveal current elemental and harmful-status resistance information for rest of battle. |
| **Awakened — Meridian Shot** | one enemy | Physical / Neutral | 275 | 125 | 30% Defense penetration; 30% Stun. |
| **Awakened — Survey the Field** | all current enemies | Information | — | — | Reveal currently knowable elemental/status and ordinary active stat changes for rest of battle; apply Perfect Route through end next Prime round: next damaging command cannot miss through ordinary Base Hit/Evasion and gains +25% final damage. Unavailable Round 3; never reveals future scripts/transformations/thresholds/random choices. |
| **Awakened — Closed Route** | one enemy | Physical / Neutral | 220 | 130 | 20% Defense penetration; 50% Stun. |
| **Legacy/final — Last Horizon** | Prime Round 3 only; all enemies | Physical / Neutral | 240 each | 120 | 30% Defense penetration; 25% Stun per successfully damaged target. After dismissal, conscious party gains Speed +15% and Base Hit +15 for 2 full normal rounds. |



## Global Prime references
- `../PRIME_SYSTEM_RULES.md`
- `../PRIME_SCALING.md`
- `../PRIME_STATUS_CONTROL.md`

## Ruin formula scope
If this Prime contains Ruin damage, use the command's explicitly authored Prime formula.
Do **not** force the character-Ability 75/25 Ruin rule onto a Prime command.

# Chapter 0 — Dialogue Authority Index

**Chapter:** 0 — Broken Convoy  
**Status:** CURRENT WORKING DIALOGUE AUTHORITY — MAINLINE P01–P07 + C01; PERSON-BRAIN PERFORMANCE PASS COMPLETE; CYANIS CHARACTER AUDIT COMPLETE; ILYRA ROLE-BALANCE CURRENT; MATURE-ADULT SPEECH / PROFANITY AUDIT CURRENT; SPOKEN-DIALOGUE/NARRATION RULE REMAINS IN FORCE  
**Combined chapter read-through:** `CHAPTER_00_REHEARSAL_FIRST_WORKING_DIALOGUE_MANUSCRIPT.md` — stale against the newly revised atomic sources  
**Rhythm authority:** `../../AGENT_SYSTEM/NATURAL_TURN_LENGTH_AND_FLOOR_HOLDING_LOCK.md`  
**Mature-adult speech authority:** `../../AGENT_SYSTEM/MATURE_ADULT_SPEECH_AND_PROFANITY_LOCK.md`  
**Spoken-dialogue authority:** `../../AGENT_SYSTEM/SPOKEN_DIALOGUE_VS_NARRATION_LOCK.md`

## Authority rule

The standalone P01–P07 and C01 rehearsal-first files in this folder are the exact scene-level production authorities.

`CHAPTER_00_REHEARSAL_FIRST_WORKING_DIALOGUE_MANUSCRIPT.md` remains a convenient historical assembled read-through, but it predates the 2026-09-12 Person-Brain performance pass and the later Ilyra role-balance corrections. Where it disagrees with a standalone file, the standalone file wins until the manuscript is regenerated.

Current sequence:
- P01 — Convoy / Opening Ambush — **Person-Brain performance pass current; Cyanis audit pass; mature-adult speech audit pass**
- P02 — Wreck Field — **Person-Brain performance pass current; Cyanis audit pass; mature-adult speech audit pass**
- P03 — Evacuation Relay Decision — **Person-Brain performance pass current; Cyanis audit pass; mature-adult speech audit pass**
- P04 — Field Triage / Ilyra / First Flare — **Person-Brain performance pass current; Cyanis audit pass; mature-adult speech audit pass; medical emphasis story-earned**
- P05 — Concealed Ruin Vanguard — **Person-Brain + Ilyra role-balance current; Cyanis audit pass; mature-adult speech audit pass**
- P06 — Riftmaw + Convoy War-Sorcerer — **Person-Brain performance pass current; Cyanis audit pass; mature-adult speech audit pass; Card-response medical emphasis story-earned**
- P07 — Aftermath / Survivor Recovery — **Person-Brain performance pass current; Cyanis audit pass; mature-adult speech audit pass; aftermath/medical emphasis story-earned**
- optional C01 — Six Minutes — **Person-Brain + Ilyra role-balance current; Cyanis audit pass; mature-adult speech audit pass**

## Cyanis character audit — COMPLETE

The Chapter-0 atomic dialogue has been checked specifically against current Cyanis character authority rather than assuming the broader Person-Brain pass covered every possible drift.

### Audit target
Cyanis must not collapse into:
- generic protagonist commander;
- exposition-question feed for more specialized characters;
- infallible decision-maker;
- constant martyr with no ordinary personality;
- person who owns every plan merely because he is the controllable lead;
- character whose humor disappears whenever plot information enters the scene.

Current active character rule:
> **Leadership is not a dialogue assignment.**

Cyanis may ask a question when he genuinely wants the answer, but he is not required to be the automatic prompt that lets another character explain the route, Card, medicine, politics, evidence, or next objective.

### P01 — Convoy / Opening Ambush
- Cyanis enters through competence, mild insubordination, dry humor, and practical attention rather than protagonist grandeur.
- he notices the empty-road problem and explains the reasoning behind his concern.
- the ranking convoy officer does not surrender authority merely because Cyanis is right to raise the concern.
- Cyanis can joke with an officer who already knows his habits.

### P02 — Wreck Field
- Cyanis gives practical rescue instructions without becoming a medical specialist.
- he can make a bounded field judgment under uncertainty rather than either knowing the north cut is bait or obediently accepting the first interpretation.
- `Mine` / `I noticed` / the `mostly` callback preserve personality inside crisis work.
- he does not solve the entire field alone; escorts and civilians retain agency and work.

### P03 — Evacuation Relay Decision
- Cyanis gives a competent complete field report rather than functioning as a checklist-answer machine.
- he argues against a chase with actual tradeoffs rather than heroic certainty.
- the officer identifies Cyanis's self-expendability pattern and keeps final command authority.
- Cyanis is capable of saying `You're right` without a defensive speech.

### P04 — Field Triage / Ilyra / First Flare
- Cyanis and Ilyra meet as professionals solving overlapping problems.
- he initially minimizes his own symptoms, then improves the information when Ilyra explains why precision matters.
- he accepts Ilyra's medical expertise without giving up operational judgment or turning her into his caretaker.
- his humor and mild embarrassment remain present even while the unexplained Card event makes him the medical subject.

### P05 — Concealed Ruin Vanguard
- Cyanis makes a tactical judgment about not being pulled away from camp rather than being assigned a generic leader speech.
- he is curious about the Vanguard's motive because **he** wants to understand what happened, not merely to feed Ilyra an exposition prompt.
- he takes water without being managed and volunteers his own symptom update from established continuity.
- the reset scene proves he can participate in an ordinary, uncertain conversation rather than only directing action.

### P06 — Riftmaw + War-Sorcerer
- high danger appropriately compresses his speech into real command language.
- his double-vision joke is a believable bad joke under stress, and he accepts Ilyra's correction when she explains why it corrupts medical information.
- he apologizes plainly and gives better information rather than performing vulnerability for the scene.
- he does not attempt to explain or classify the Card just because he is the protagonist.

### P07 — Aftermath / Survivor Recovery
- Cyanis argues for one bounded survivor sweep because the immediate human consequence matters to him, not because he must solve every missing-person problem indefinitely.
- he accepts the officer's boundary and does not turn the sweep into pursuit.
- he does not interpret Ilyra staying as devotion or debt; when corrected, he accepts her professional reason.
- his concern about whether Ilyra's patients are covered is consistent with his responsibility pattern without becoming ownership of her decision.

### C01 — Six Minutes
- this remains the strongest Chapter-0 proof that Cyanis is pleasant company rather than merely useful company.
- he starts and sustains jokes, argues over shared blame, laughs at Ilyra's bad joke, admits an embarrassing habit, notices her `find a task` posture because he recognizes the same behavior in himself, and invents the arbitrary six-minute rule.
- the scene contains no plot-question function for him at all.
- his ordinary personality survives when command pressure disappears.

### Result
**No Chapter-0 atomic dialogue rewrite was required by the Cyanis-specific audit.**

The existing current scenes already satisfy his character authority. The audit instead adds a forward guardrail: future dialogue must preserve Cyanis as a social person with opinions, humor, wants, mistakes, and ordinary interests rather than using him mainly as the leader who asks the next useful question.

## Mature-adult speech / profanity audit — CURRENT

Owning rule:
- `../../AGENT_SYSTEM/MATURE_ADULT_SPEECH_AND_PROFANITY_LOCK.md`

Chapter 0 has been checked against the new adult-speech standard as part of the Cyanis audit and current Ilyra role-balance state.

Current interpretation:
- the cast are mature adults and do not need to be sanitized into uniformly polite JRPG dialogue;
- profanity remains character-specific rather than quota-driven;
- Cyanis is allowed to swear, joke crudely, or use blunt language where his state and relationship make it natural, but Chapter 0 does not require extra profanity merely to prove maturity;
- Ilyra's occasional profanity and sharper humor are allowed to coexist with professional medical competence;
- crisis command language may be clean because clarity matters, not because the script is censoring the characters;
- the convoy officer's authority does not require everyone around him to sound artificially formal;
- C01 remains intentionally ordinary adult conversation rather than sanitized protagonist/healer bonding.

**No Chapter-0 atomic rewrite was required solely for profanity.** The current dialogue already sounds adult without needing mechanical swear insertion.

## Maevra familiarity / rank forward lock

Current character authority now explicitly establishes:
> **Maevra already knows Cyanis and Ilyra very well as separate people. She is still their senior commander.**

This matters primarily to Chapters 1–3 rather than Chapter 0's current atomic scene set, because Maevra is not inserted into Cyanis/Ilyra's immediate Chapter-0 dialogue merely to demonstrate relationship history.

Future early-chapter dialogue must therefore preserve both sides simultaneously:
- Maevra may tease Cyanis or Ilyra from real prior familiarity;
- she may know their habits, tells, objections, and likely reactions;
- Cyanis and Ilyra may use established shorthand with her rather than sounding newly acquainted;
- operationally, Maevra's direct orders still carry senior-command authority;
- disagreement and familiarity do not make them equal-rank buddies;
- rank does not require artificial emotional distance, stiff formality, or sanitized language.

The correct early read is **familiarity plus hierarchy**.

This relationship is separate from Maevra/Torren's much longer private history and its own reveal timing.

## Person-Brain performance pass — COMPLETE

This pass supersedes the earlier assumption that crisis dialogue should remain clipped merely because the scene is under pressure.

Current rule:
- true commands, immediate danger calls, and battle warnings may remain extremely short;
- field reports, tactical judgments, treatment reasoning, disagreements, and quiet scenes may hold the floor for as long as the person and situation require;
- interruptions must come from motive, urgency, expertise, humor, or relationship rather than from an artificial sentence-length ceiling;
- different speakers keep different rhythms;
- longer speech may not be used to repeat information the player has just seen.

### P01 — Convoy / Opening Ambush
- Cyanis and the convoy officer sound familiar with each other's habits before the attack;
- Cyanis explains why the empty road bothers him rather than supplying only a clue fragment;
- once the ambush begins, language appropriately collapses back into commands.

### P02 — Wreck Field
- rescue exchanges give civilians/escorts enough reasoning to act without turning Cyanis into a medical specialist;
- the north-cut discussion becomes an actual disagreement about uncertainty and pursuit rather than one-clause question/answer ping-pong;
- the officer asks Cyanis to bring a broader field picture before the next decision.

### P03 — Evacuation Relay Decision
- Cyanis delivers one competent linked field report instead of waiting for checklist prompts;
- the prior spoken-dialogue correction remains intact: `You said that. What changes now?` turns the argument toward the five people who still need carrying rather than repeating the north-cut evidence;
- the officer challenges Cyanis's habit of objecting to needless exposure and then volunteering himself for the dangerous option;
- formal authority remains with the officer.

### P04 — Field Triage / Ilyra / First Flare
- Cyanis and Ilyra meet as two professionals solving overlapping problems, not leader + healer functions;
- Ilyra completes medical/practical reasoning instead of yielding after each observation;
- Cyanis initially minimizes his symptoms, then gives better information when Ilyra explains why precision matters;
- Ilyra joins the defense through her own Warden judgment, not a recruitment exchange;
- the heavy medical content is intentional here because Ilyra is introduced while actively running triage and the Card's first physical correlation occurs in the same beat.

### P05 — Concealed Ruin Vanguard
- the hostile approach stays terse where danger earns it;
- Ilyra's decision not to chase comes from tactical judgment rather than healer-coded caution;
- Cyanis takes water and volunteers his own symptom update instead of Ilyra managing both actions;
- Ilyra gets a nonmedical reasoning beat about uncertainty: she hates not knowing but hates inventing an answer because uncertainty is irritating even more;
- their early humor becomes more comfortable without implying instant intimacy.

### P06 — Riftmaw + War-Sorcerer
- Ilyra refuses vague symptom language during the second Card response because precision materially affects her decision;
- Cyanis's double-vision joke produces a clear adult boundary: joking corrupts medical information she cannot safely guess about;
- Cyanis apologizes plainly and answers accurately without turning the scene into confession;
- medical emphasis remains justified because this is the second unexplained Card response and the physical correlation is a core Chapter-0 story fact.

### P07 — Aftermath / Survivor Recovery
- the previous spoken-dialogue correction remains intact: Ilyra states her decision to stay once and supplies the reason only after Cyanis challenges the premise;
- her medical report and reason for remaining overnight are story-earned rather than default dialogue assignment;
- the exhausted officer is allowed dry humor and reduced polish after the crisis;
- the survivor sweep remains bounded and no pursuit is added.

### C01 — Six Minutes
- the bitterleaf mistake remains the opening joke, but the scene deliberately leaves medicine behind instead of spending the whole optional scene on food/rest/patient management;
- Cyanis and Ilyra exchange first impressions, embarrass one another, argue about blame, and let Ilyra laugh, swear, tease, and make a bad joke she immediately regrets;
- their shared tendency to invent work for themselves remains visible without becoming diagnosis or therapy;
- the six-minute rule remains an arbitrary shared refusal to turn rest into another task;
- C01 is the Chapter-0 proof that both Cyanis and Ilyra have ordinary social identities outside command/medical functions.

## Ilyra dialogue-assignment boundary

Current active Ilyra character authority locks:
> **Grace is not a dialogue assignment.**

Chapter 0 necessarily introduces Ilyra in a medical crisis, so P04/P06/P07 contain real treatment and Card-response work. That does **not** license every later Ilyra turn to be medical, protective, therapeutic, or caretaker-coded.

Current Chapter-0 interpretation:
- medicine belongs to Ilyra when the scene genuinely requires a Warden/physician;
- tactical uncertainty, jokes, profanity, first impressions, irritation, ordinary curiosity, and social friction also belong to her;
- Cyanis can learn to volunteer symptom information rather than waiting for Ilyra to administer a checklist;
- other characters can notice danger, injury, exhaustion, or human consequences first;
- quiet/optional material must prove that Ilyra remains a whole person once the medical emergency stops.

This boundary carries forward into all later chapters.

## Spoken-dialogue / narration rule remains in force

Owning rule:
- `../../AGENT_SYSTEM/SPOKEN_DIALOGUE_VS_NARRATION_LOCK.md`

Core interpretation:
> **The environment shows. Evidence owners interpret. Authority figures decide. Characters react. Nobody recites the scene back to the player.**

The Person-Brain pass does not reverse the earlier narration audit. Longer turns are used for judgment, reasoning, disagreement, humor, or human texture—not for recap.

## Protected continuity retained

- the Card is cased and inert through P01–P03;
- first incomplete green/gold response occurs in P04 and fully ends there;
- concealed Ruin Vanguard remains unidentified and withdraws alive in P05;
- second incomplete response occurs in P06;
- recovery casing breaks during P06 and leaves continuity;
- Cyanis carries the intact dark Card afterward;
- no deliberate separation test occurs in Chapter 0;
- no Prime, Might, Last Sentinel, bearer, ancient-weapon, or Entity explanation is revealed;
- Ilyra independently stays overnight after P07;
- C01 remains optional and plotless;
- Chapter 0 ends only when the player explicitly continues to Brackenwall.

## Current synchronization state

The atomic dialogue sources changed during the Person-Brain performance pass, and P05/C01 changed again during the Ilyra role-balance pass.

The Cyanis-specific character audit and mature-adult speech/profanity audit did **not** require another atomic dialogue rewrite.

Therefore:
- standalone atomic sources are the current exact wording authority;
- `CHAPTER_00_REHEARSAL_FIRST_WORKING_DIALOGUE_MANUSCRIPT.md` is stale until rebuilt;
- any spoiler-free reader/novelization created before this pass is stale for Chapter-0 exact dialogue until regenerated after the broader Chapters 0–3 dialogue pass.

## Conflict order

If Chapter-0 dialogue sources disagree:
1. current Chapter-0 story authority and later explicit user corrections;
2. current character authorities for Cyanis, Ilyra, and supporting characters, including Cyanis's dialogue-assignment guardrail, Ilyra's dialogue-assignment guardrail, and Maevra familiarity/rank authority where relevant;
3. current standalone rehearsal-first scene authority;
4. `../../AGENT_SYSTEM/MATURE_ADULT_SPEECH_AND_PROFANITY_LOCK.md`, `../../AGENT_SYSTEM/SPOKEN_DIALOGUE_VS_NARRATION_LOCK.md`, `../../AGENT_SYSTEM/NATURAL_TURN_LENGTH_AND_FLOOR_HOLDING_LOCK.md`, and other current workflow locks for production interpretation;
5. `CHAPTER_00_REHEARSAL_FIRST_WORKING_DIALOGUE_MANUSCRIPT.md` only where its wording has been explicitly resynchronized;
6. older/historical dialogue only as provenance.

> **Chapter 0 Person-Brain performance pass, Cyanis character audit, Ilyra role-balance pass, and mature-adult speech/profanity audit are complete. Atomic files are exact wording authority; combined manuscript and prior novelization are stale until the Chapters 0–3 pass is finished and synchronized.**

...
**Combined read-through:** `CHAPTER_02_REHEARSAL_FIRST_WORKING_DIALOGUE_MANUSCRIPT.md` — stale where atomic sources have changed
...
The combined manuscript was generated before later numbering and dialogue revisions and therefore must not override a newer atomic source.
...
6. `BEAT_06_WESTERN_MAP_MURAL_NATURAL_TURN_WORKING_DIALOGUE.md` — spoken/narration audit current
...
10. `BEAT_10_PRISONER_GALLERIES_FIRST_CONTACT_NATURAL_TURN_WORKING_DIALOGUE.md` — spoken/narration audit current
...
13. `BEAT_13_RHAZEK_MASKED_OFFICER_REHEARSAL_FIRST_WORKING_DIALOGUE.md` — spoken/narration audit current
14. `BEAT_14_COMMANDER_RHAZEK_BASTION_MASTER_REHEARSAL_FIRST_WORKING_DIALOGUE.md` — spoken/narration audit current
15. `BEAT_15_RETURN_TO_THE_PRISONERS_RELEASE_REHEARSAL_FIRST_WORKING_DIALOGUE.md`
16. `BEAT_16_RETURN_TO_DUNMERE_ROAD_REOPENED_CLEANUP_REHEARSAL_FIRST_WORKING_DIALOGUE.md` — spoken/narration audit current
...
Prior material rhythm corrections:
- Beat 2 — witness accounts / Bastion-road significance;
- Beat 5 — Ancient-complex entrance observations;
- Beat 6 — western mural investigation;
...
Material revisions:
- **Beat 2** — the idle loaded wagons visually establish that the road has stopped; Maevra states only the operational conclusion instead of inventorying harnesses, teams, and wagon state aloud.
- **Beat 5** — Ilyra and Torren keep distinct expertise-based observations; Maevra no longer summarizes their visible evidence back into a second spoken explanation of the room.
- **Beat 6** — the wall-scale map gets the first word visually; the cast no longer opens with `That's a map / Looks like one`. The scene moves directly into Wayfinder comparison and genuine geographic interpretation.
- **Beat 10** — prisoner testimony establishes onward movement once; Maevra identifies `Transit point.` and the party moves forward instead of restating the same conclusion three ways. Rhazek's identification changes the target rather than triggering another policy recap.
- **Beat 13** — Torren does not re-litigate the prisoner testimony with Rhazek, and Rhazek no longer recites the player's dungeon progress back to the player. His judgment is now: `You disrupted my fort. That is not the same as taking it.`
- **Beat 14** — after Rhazek visibly orders a general withdrawal, Cyanis no longer announces the withdrawal back to him. He immediately asks about the unresolved prisoners instead.
...
- Chapter 2 begins outside Dunmere after explicit Chapter-1 advance.
- The Old Bastion is the current fort name; `Red Transfer Bastion` is retired.
- Cyanis + Ilyra + Torren are the permanent combat party.
- Maevra remains physically present but fully non-playable/noncombat.
- The western mural establishes the larger route system, ancient city at modern Caelora, and separate northern structure bearing the exact modern Crest of Yahtrea without explaining them.
- The current prisoners are freed from the Old Bastion.
- The Bastion is established as a holding/transit site; earlier transferred captives remain missing.
- Rhazek personally owns the local seizure policy, is defeated locally, survives a credible withdrawal, and loses the Bastion.
- Greenhollow ↔ Dunmere ordinary travel reopens.
- Chapter 3 begins only through explicit player advance confirmation.

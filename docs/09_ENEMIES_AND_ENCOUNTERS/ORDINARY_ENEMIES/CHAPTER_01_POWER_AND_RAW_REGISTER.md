# Chapter 1 — Ordinary Enemy Power + Raw Register

**Status:** **CHAPTER-1 ORDINARY ENEMY POWER PASS COMPLETE**

These Chapter-1 ordinary raw bodies are:
> **newly authored active-balance values**

They were not recovered from Audit129–135.

Individual enemy files remain the owning authority.

| Enemy | Lv | HP | ATK | MAG | DEF | Spirit | SPD | EVA | SR | Direct-damage Powers |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| Black Host Raider | 2 | 120 | 26 | 10 | 14 | 12 | 22 | 0 | 0 | 120 / 145 |
| Black Host Crossbowman | 2 | 95 | 25 | 10 | 12 | 12 | 24 | 5 | 0 | 125 / 155 |
| Ruin Shieldbearer | 3 | 165 | 25 | 18 | 23 | 19 | 17 | 0 | 5 | 115 / 135 |
| Greenhollow Stalker | 3 | 135 | 29 | 10 | 15 | 13 | 27 | 5 | 0 | 140 / 125 |
| Thornvine Creeper | 3 | 155 | 23 | 24 | 18 | 18 | 15 | 0 | 5 | 115 / 120 |
| Briar Boar | 4 | 195 | 32 | 8 | 22 | 15 | 21 | 0 | 0 | 160 / 145 |
| Hollow Watch Sentry | 3 | 130 | 24 | 26 | 20 | 21 | 24 | 0 | 10 | 120 |
| Hollow Watch Ballista | 4 | 170 | 40 | 0 | 22 | 18 | 18 | 0 | 10 | 135 / marked 200 |

## Current placement boundary

Active Chapter-1 ordinary roster:
- Briar Passage: Greenhollow Stalker / Thornvine Creeper / Briar Boar
- Hollow Watch Black Host: Black Host Raider / Black Host Crossbowman / Ruin Shieldbearer
- Hollow Watch constructs: Hollow Watch Sentry / Hollow Watch Ballista

Brackenwall Reaver's authored body remains retained in its individual file, but it is **not part of the active Chapter-1 roster or Chapter-1 formation validation** after the 2026-09-22 roster trim.

Chapter-1 authored/protected:
- Briarhide Stalker — **PASS**
- Black Host Irritant Fitting — **POWER COMPLETE**

Chapter-1 optional Elite:
- Watch Captain Frame — **PASS**

## Status rollout
Chapter 1 does not introduce Burn, Stun, or Freeze.

Bleed is the only harmful-status rider used by this batch.

Hollow Watch constructs are explicitly Bleed Immune.

## Carryover rule
A recurring identity may retain its action grammar later, but:
> **late chapter reuse does not automatically inherit the Chapter-1 raw body**

Chapter-appropriate raw scaling remains required.


## v77 difficulty validation
The Chapter-1 ordinary roster was retested against actual route levels rather than end-Chapter-1 Lv5.

Result:
> **PASS — NO ORDINARY RAW/POWER CHANGES**

Watch Captain Frame is separately adjusted in its Elite owner file: **HP 820 → 500**.

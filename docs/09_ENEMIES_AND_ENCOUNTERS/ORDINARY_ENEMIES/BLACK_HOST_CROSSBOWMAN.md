# Black Host Crossbowman

**Current certified use:** Chapters 0–1 carryover; both placements current  
**Status:** **CH1 POWER COMPLETE / CH1 RAW BODY AUTHORED**

## Chapter-1 body
| Lv | HP | ATK | MAG | DEF | Spirit | SPD | EVA | SR |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 2 | 95 | 25 | 10 | 12 | 12 | 24 | 5 | 0 |

## Actions

### Crossbow Bolt
- one party member
- Physical / Neutral
- **125 Power**
- Base Hit **95**
- no harmful-status rider

### Aimed Bolt
- one party member
- Physical / Neutral
- **155 Power**
- Base Hit **95**
- **10% Bleed**
- 1-round repetition lock

## Carryover boundary
Chapter-0 and later variants are not silently scaled from this Chapter-1 line.

## Chapter-0 body

| Lv | HP | ATK | MAG | DEF | Spirit | SPD | EVA | SR |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | **72** | **21** | 8 | 10 | 10 | 22 | 5 | 0 |

Chapter-0 actions:
- Crossbow Bolt — **115 Power**, Physical / Neutral, Base Hit100
- Aimed Bolt — **135 Power**, Physical / Neutral, Base Hit95, 1-round repetition lock

No Bleed rider in Chapter 0.


## Chapter-0 placement lock
Black Host Crossbowman is part of the locked Chapter-0 roster. Current authored use: **P01 Combat 1 and P02 Mixed Pressure**.

## Chapter-1 behavior lock
- Uses the normal action-selection fallback between currently legal Crossbow Bolt and Aimed Bolt.
- Aimed Bolt keeps its Chapter-1 repetition lock.
- Single-target selection is equal among conscious active party members.
- No back-row/healer priority is inferred from the ranged identity.
- Formation role: lighter ranged body whose danger comes from remaining active beside other Host roles.

## Chapter-0 behavior lock
- Uses the normal action-selection fallback between currently legal Crossbow Bolt and Aimed Bolt.
- Aimed Bolt retains its 1-round repetition lock.
- No Bleed rider is active in Chapter 0.
- P01/P02 uses are Cyanis-solo, so all legal single-target actions necessarily target Cyanis.
- No hidden distance/back-row targeting rule.

## Visual authority
Current locked Chapter-0 appearance:
> `../../14_ART_AND_VISUALS/ENEMIES/CHAPTER_00_ENEMY_VISUAL_AUTHORITY.md`

Combat/encounter mechanics remain owned by this file; visual identity is owned by the art-domain authority above.

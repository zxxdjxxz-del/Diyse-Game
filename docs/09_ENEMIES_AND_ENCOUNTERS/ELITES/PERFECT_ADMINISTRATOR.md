# Perfect Administrator
**Chapter:** 11 optional Elite  
**Architecture:** one HP bar  
**Status:** **POWER COMPLETE / RAW LINE RETAINED / FORMALLY VALIDATED v87**

| Lv | HP | ATK | MAG | DEF | Spirit | SPD | EVA | SR |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 55 | **5,800** | **180** | **198** | **137** | **141** | **52** | 5 | 10 |

True construct: **Bleed Immune**

### Perfect Judgment
- Hybrid / Neutral /50% ATK /50% MAG / one target
- **310 Power**, Base Hit100

### Administrative Ruin
- Magical / Ruin / one target — **300 Power**, Base Hit100

### Perfected Directive
- Magical / Colorless / all conscious party members
- **220 Power per target**, Base Hit95, 2-round repetition lock

### Enforcement Sequence
- Hybrid / Neutral /50% ATK /50% MAG / one target
- **2 × 165 Power = 330 total**
- Base Hit100 per hit
- 2-round repetition lock

### Absolute Procedure
> **Power: N/A — no direct damage**
- Attack +10%
- Magic +10%
- Base Hit +10
- +10 Total Defense
through end following round
- no extra action
- 2-round repetition lock

Administrative terminology cannot remove commands, delete levels/classes/equipment/Cards/abilities, alter save data, or create a Permission/Authority resource.

One body. No support wave. No transformation. No extra ordinary action.


## v87 balance certification
Validated against approximately Lv49–51 mandatory-route access and Lv59–61 completionist access. Retain 5,800 HP and all current Powers.

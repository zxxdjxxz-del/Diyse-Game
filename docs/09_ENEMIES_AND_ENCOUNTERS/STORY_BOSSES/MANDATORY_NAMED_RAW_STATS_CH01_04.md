# Diyse — Mandatory Named Raw Stats — Chapters 1–4
**Migration baseline:** `Diyse_CURRENT_WORKING_TRACKER_CONSOLIDATED_2026-08-27_v85.md`  
**Primary enemy-production authority:** compatible **Audit90 / Audit93** plus accepted later tracker roster/action cleanups.  
**Primary raw-stat authority:** **Audit129 + Audit130 / Audit131 / Audit132 / Audit133 / Audit134 / Audit135**.  
**Current whole-project written authority:** **v2.20 / Audit135**.  
**Migration rule:** later current names, chapter reindexing, four-element rules, removed-system firewalls, and fresh-body Prime-refresh rules supersede stale earlier enemy text.


| Ch | Encounter / form | Lv | HP | ATK | MAG | DEF | Spirit | SPD | EVA | SR | Architecture |
|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | **Thornhide** | 4 | 850 | 34 | 18 | 22 | 20 | 29 | 10 | 0 | nonlethal; Irritant Fitting objective / 25% protected floor |
| 1 | **Watch Castellan** | 6 | 450 | 42 | 27 | 27 | 24 | 25 | 0 | 5 | one bar; Fortress → Walking same-bar |
| 2 | **Archive Leviathan** | 9 | 1,900 | 50 | 52 | 31 | 33 | 25 | 0 | 5 | one bar; Recorded Pattern → Emergent same-bar |
| 2 | **Commander Rhazek — Bastion Master** | 10 | 2,050 | 58 | 44 | 36 | 32 | 27 | 5 | 5 | one bar; Bastion Master → Ruin Escalation same-bar; survives/withdraws |
| 3 | **First Command Warden** | 14 | 2,850 | 72 | 72 | 43 | 43 | 30 | 0 | 10 | one bar; Imposed Authority → Challenged Authority same-bar |
| 4 | **Elder Thornhide** | 14 | 2,100 | 72 | 28 | 60 | 55 | 36 | 10 | 5 | mandatory nonlethal; fixed 4-round Last Sentinel sequence |
| 4 | **Reaction Conduit** | 17 | 2,400 | 58 | 80 | 50 | 55 | 33 | 5 | 5 | mandatory nonlethal stabilization; four-expression cycle |
| 4 | **Regulation Crucible — Form I** | 18 | 2,400 | 64 | 90 | 54 | 57 | 30 | 0 | 10 | Form I; four targetable elemental chambers |
| 4 | **The Seventh Reaction — Form II** | 19 | 2,900 | 72 | 98 | 58 | 62 | 32 | 0 | 10 | genuine fresh Form II; inherits surviving chamber traits |

## Authority notes
- Ch4 Regulation Crucible uses Audit130's corrected **2,400 HP** Form-I value.
- Genuine fresh forms use current Prime-refresh authority.
- Same-bar states do not refresh Prime availability.


## Watch Castellan working recertification override
The former Audit129-compatible line:
> Lv6 / 1,758 HP / 36 ATK / 27 MAG / 27 DEF / 24 Spirit / 25 SPD

was tested against the wrong late Chapter-1 reference point.

The actual S008 balance profile is approximately:
- Cyanis Lv2 / Crest Knight CL1
- Ilyra Lv2 / Blue Warden CL1
- Maevra Lv2 guest
- Lv3 high-side validation

Current working recertification line:
> **Lv6 / 450 HP / 42 ATK / 27 MAG / 27 DEF / 24 Spirit / 25 SPD / 0 EVA / 5 SR**

This is the active working balance line for the boss pass. It remains a working recertification result pending later formal canon promotion.


## Archive Leviathan — v97 true-battle certification
Actual S013 progression:
- Chapter 2 begins at ~Lv5;
- Archive Leviathan is the first named/story EXP package in the chapter;
- central mandatory route reaches it around **Lv6**;
- completionist/high-side route reaches it around **Lv7**.

Recovered Lv6 player throughput against DEF31:
- Cyanis basic: **51.24**
- Ilyra basic: **37.80**
- Torren basic: **58.58**
- Maevra basic: **30.86**
- all-basic party: **178.48 per round**

Inherited HP **2,592** requires **14.52 all-basic rounds** before healing/mechanic tax.

Current working HP:
> **1,900**

Expected pacing:
- mandatory normal: **~9–10 rounds**
- mandatory aggressive: **~8–9 rounds**
- completionist/high-side: **~8–9 rounds**
- safety / heavy Recorded-Pattern response: **~10–11 rounds**

Preserve:
- Lv9 encounter identity;
- DEF31;
- Spirit33;
- SPD25;
- EVA0;
- SR5;
- one HP bar;
- Recorded Pattern forms only from an actor repeating the same exact eligible direct-damage action on consecutive offensive actions;
- same-bar emergence;
- no refill;
- no transition damage;
- no Prime refresh;
- no Water element.

Recovered signatures:
- Leviathan Rend — Physical / Neutral; Base Hit 100; 25% Bleed; 1-round repetition lock.
- Vault Crash — all conscious party members; Physical / Neutral; Base Hit 95; 15% Staggered per target; 2-round repetition lock.

### Raw-offense certification
Current:
> **ATK50 / MAG52**

Direct-damage Powers:
- Leviathan Rend — **220**
- Vault Crash — **140 per target**
- Archive Undertow — **150 per target**
- Recorded Pattern — **N/A; adaptation only; 20% later-repeat final direct-damage reduction once the deterministic pattern forms**

The direct-damage coefficient gap is closed.


True-battle certification (v97):
- mandatory Lv6 prepared: **100% wins / median 10 / mean 10.05 / 0.05% any-KO** over 20,000 runs;
- completionist/high-side Lv7 prepared: **100% wins / median 9 / mean 8.58 / 0% any-KO** over 20,000 runs;
- **HP / raw stats / Powers retained**.


## Commander Rhazek — Bastion Master working recertification

Actual S015 route:
- mandatory central: **~Lv7**
- completionist: **~Lv8**
- high-side optional route can approach **Lv9**

Current working line:
> **Lv10 / 2,050 HP / 58 ATK / 44 MAG / 36 DEF / 32 Spirit / 27 SPD / 5 EVA / 5 SR**

Direct-damage Powers:
- Commander's Cut — **180**
- Shieldline Break — **220**
- Command-Link Pulse — **135 per target**
- Crossfire Bolt — **165**
- Ruin-Driven Cut — **210**
- Bastion Breaker — **250**
- Ruin Sweep — **145 per target**

Non-damaging:
- Hold the Bastion — Power N/A
- Shield Screen — Power N/A
- Reinforced Advance — Power N/A

Finite support:
- Bastion Shield Detachment — 180 HP
- Bastion Ranged Position — 150 HP

Expected pacing:
- mandatory normal **~10–11 rounds**
- completionist **~8–9**
- high-side ~7–8

The 45% HP Ruin/armor shift is same-bar and does not refresh Prime availability.


## First Command Warden working recertification

Actual S020 route:
- mandatory pre-battle EXP **11,280** = Lv11, 820 short of Lv12
- completionist fixed-content EXP **14,280** = Lv12, only 120 short of Lv13
- incidental optional combat can place the high-side route around Lv13

Current working line:
> **Lv14 / 2,850 HP / 72 ATK / 72 MAG / 43 DEF / 43 Spirit / 30 SPD / 0 EVA / 10 SR**

Power block:
- Authority Lance — 190
- Judgment Pulse — 140 per target
- Seal Reprisal — 100
- Major Ruling — 210 per target
- Recorded Physical/Magical/Hybrid Analogue — 150 single / 105 per target AoE
- Warden Crush — 245
- Challenged Verdict — 230
- Command Collapse — 165 per target

Command Seal is Power N/A and never removes a command.

Command Ring:
- 260 HP while aligned
- 40 DEF / 46 Spirit
- one-round Preparation window
- disrupting the Ring cancels Major Ruling
- may align again after 3 full rounds

Expected:
- mandatory normal ~10–11
- completionist ~8–9
- high-side Lv13 ~7–8

State B begins at 45% HP with no refill and no Prime refresh.


## Elder Thornhide working recertification

Actual S022 route:
- mandatory **Lv13**
- completionist fixed-content **Lv15 exactly**
- high-side optional/incidental **~Lv16**

Current line:
> **Lv14 / 2,100 HP / 72 ATK / 28 MAG / 60 DEF / 55 Spirit / 36 SPD / 10 EVA / 5 SR**

Power block:
- Territorial Rush — **190**
- Scarred Maul — **225**
- Briar Sweep — **125 per target**
- Guard the Den — Power N/A
- Warning Display — Power N/A

Round 4:
- forced Recovered Last Sentinel
- Sentinel Impact — **340 Power / 40% Defense penetration**
- Elder Thornhide retreats alive

Duration:
> **exactly 4 rounds on mandatory and completionist routes**

This is a protected scripted-duration exception. Optional progression improves safety/resource pressure rather than shortening the encounter.


## Reaction Conduit working recertification

Actual S023 route:
- mandatory pre-battle **15,350 EXP = Lv13**
- completionist fixed-content **20,550 EXP = Lv15**
- thorough lower-gallery route may approach **Lv16**

Current line:
> **Lv17 / 2,400 HP / 58 ATK / 80 MAG / 50 DEF / 55 Spirit / 33 SPD / 5 EVA / 5 SR**

Expression cycle:
> Fire → Ice → Lightning → Earth

Power block:
- Reaction Pressure — **180**
- Spillway Burst — **120 per target**
- Exposure Thrash — **160**
- Instinctive Guard — Power N/A

Reaction Pressure riders:
- Fire — 20% Burn
- Ice — 20% Freeze
- Lightning — 15% Stun
- Earth — 20% Staggered

Expected:
- mandatory normal ~8–9
- completionist ~6–7
- high-side ~5–6

At 0 HP the researcher is stabilized alive.


## Regulation Crucible → The Seventh Reaction working recertification

Actual S024 route:
- mandatory pre-boss **21,462 EXP = Lv15**, 1,038 short of Lv16
- completionist fixed-content **26,662 EXP = Lv17**, 2,238 short of Lv18
- high-side with Annex Duelist/incidental combat **~Lv18**

### Form I
> **Lv18 / 2,400 HP / 64 ATK / 90 MAG / 54 DEF / 57 Spirit / 30 SPD / 0 EVA / 10 SR**

Powers:
- Regulation Slash 185
- Pairwise Return 100 × up to 2
- Compensating Drive 220
- Core Discharge 135 per target
- Regulator Guard N/A

Chambers:
- Fire / Ice / Lightning / Earth
- 300 HP each / 48 DEF / 50 Spirit
- exactly two scheduled active at once
- no independent turns / Power N/A
- destroyed remain destroyed

### Form II
> **Lv19 / 2,900 HP / 72 ATK / 98 MAG / 58 DEF / 62 Spirit / 32 SPD / 0 EVA / 10 SR**

Powers:
- Unified Impact 205
- Compression Lance 245
- Seventh Confluence 160 per target
- Overflow Equation 2 × 115
- Central Rebalance N/A

Fresh-body transition:
- no spillover
- no intermediate reward
- no chamber respawn
- Prime availability refreshes

Expected complete encounter:
- mandatory **~13–15 rounds**
- completionist **~10–12**
- high-side **~9–11**


## Thornhide Power-complete override
Raw body remains:
> Lv4 / HP850 / ATK34 / MAG18 / DEF22 / Spirit20 / SPD29 / EVA10 / SR0

Direct damage:
- Panic Rush155
- Briar Rend135 / 20% Bleed
- Cornered Sweep110 AoE
- Frightened Guard N/A

Black Host Irritant Fitting:
- HP180 / DEF18 / Spirit18
- Power N/A
- required nonlethal objective

While fitting survives:
> Briarhide floor = **213 HP / 25% Max HP**

Fitting 0 HP ends battle immediately with Briarhide retreat alive.

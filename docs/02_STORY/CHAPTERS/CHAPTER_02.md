# Chapter 2 — The Drowned Oath

**Current whole-project written authority:** **v2.20 / Audit135**, plus later explicit user corrections/current domain migrations.  
**Domain rule:** this file owns mandatory Chapter-2 story structure, scene order, reveal order, recruitment state, knowledge changes, and story outcomes. Exact dialogue belongs to the current rehearsal-first Agent Brain pipeline in `03_DIALOGUE`; combat mechanics belong to battle/encounter authority.

**Status:** **LOCKED REVISED CHAPTER-2 STORY STRUCTURE / 15 ACTIVE MANDATORY BEATS + C05. LEGACY B06 MAP-MURAL SLOT RETIRED.**

## Dialogue-production rule
Chapter 2 uses the locked Chapter-0/1 production method:

> **scene/world state → independent Person Agent Brain rehearsals → Dialogue Editor → invisible Canon/Knowledge Checker → economical HD-2D staging/implementation**

Owning workflow lock:
`docs/03_DIALOGUE/AGENT_SYSTEM/CHAPTER_0_1_PIPELINE_CONTINUITY_LOCK.md`

## Naming lock — Old Bastion
> **The Chapter-2 Black Host fort is called the Old Bastion.**

`Red Transfer Bastion` is retired terminology for current Chapter-2 story and dialogue. Historical/migration material may still contain the retired wording, but current production must use **Old Bastion** or **the Bastion** when context is clear.

---

# Chapter-wide locks

## Chapter boundary / opening
- Chapter 1 ends at the Junction/Wayfinder-area cleanup camp.
- The player explicitly confirms **Start Chapter 2**.
- Chapter 2 begins with Cyanis, Ilyra, Torren, and Maevra already on the path immediately outside Dunmere.
- The chapter does not replay the camp departure or the full journey from the Junction.

## Party state
- Commandable combat party: **Cyanis + Ilyra + Torren**.
- **Maevra remains fully non-playable and noncombat, but physically travels with the group throughout Chapter 2.**
- Maevra remains present through the Dunmere return / inn cleanup period.

## Route / world state
- Dunmere was already the intended next stop before the Wayfinder discovery.
- The group carries physical copies of useful Wayfinder information.
- Caelora remains the larger journey destination.
- People leaving Dunmere toward Caelora are being kidnapped by the Black Host.
- The direct **Greenhollow–Dunmere road** is unsafe because the Black Host occupies the **Old Bastion**, an old fort positioned to control the corridor.
- The Chapter-2 finale breaks the Old Bastion's local Black Host control and reopens direct Greenhollow ↔ Dunmere travel.
- Dunmere is the Chapter-2 cleanup hub.

## Retired opening premise
The old **Poisoned Waterworks** storyline is retired. Do not restore poisoned/contaminated Dunmere water, deliberate contamination, the three-district/shared-lower-feed investigation, lower-feed shutdown resolution, contamination residue breadcrumb, or the old S012 poisoned-water causal chain.

The waterworks remain important only as a covert route toward the Old Bastion and the older structure beneath it.

## Retired middle/finale material
Do not restore:
- a separate transfer-records story beat between the Archive and Prisoner Galleries;
- required `thirty-one transfers` clue;
- Hold the Junction;
- Extraction Causeway defense;
- surprise post-extraction combat, Rhazek rematch, hidden boss, or antagonist sting;
- dedicated playable prisoner-escort beat;
- separate road-secured confirmation beat before returning to Dunmere.

Once the prisoners are freed and organized inside the defeated Old Bastion, the story may transition directly back to Dunmere. The successful evacuation and reopened road are confirmed there.

---

# Locked 16-Beat Structure

## Beat 1 — Path Outside Dunmere
Purpose: clean Chapter-2 opening immediately outside Dunmere.

Starting state:
- Cyanis, Ilyra, Torren, Maevra present;
- combat party Cyanis + Ilyra + Torren;
- Wayfinder copies carried;
- Caelora remains the larger objective.

Mandatory:
- short real approach traversal;
- Dunmere near and functioning;
- Torren genuinely guides;
- no forced recap;
- no missing-traveler/Black Host explanation before entering town.

End → Dunmere / Beat 2.

---

## Beat 2 — Dunmere Arrival / Missing Travelers
Purpose: establish functioning Dunmere, then reveal the human problem.

Mandatory:
- outbound travel toward Caelora is under pressure;
- people leave and fail to arrive;
- direct survivor/local evidence establishes **Black Host kidnapping**, not ordinary banditry or wilderness disappearance;
- the direct Greenhollow–Dunmere road is unsafe because the Black Host occupies the **Old Bastion**;
- the Caelora journey is temporarily subordinated to the rescue/road problem.

End → seek actionable local knowledge from the Dunmere elder / Beat 3.

---

## Beat 3 — Dunmere Elder / Hidden Approach
Purpose: reject a wasteful frontal assault and reveal the covert waterworks approach.

The elder may know:
- the Old Bastion controls the corridor;
- ordinary road approaches are exposed;
- an old, little-used waterworks service route once ran toward/under the Old Bastion's lower structure.

The elder does **not** know about the Sunken Archive, Leviathan, secret passage, Prisoner Galleries, Rhazek's command-room situation, or Seyrik.

Decision:
**enter the old waterworks → find a covert way into the Old Bastion → locate the kidnapped people**.

End → Beat 4.

---

## Beat 4 — Old Waterworks
Purpose: covert infiltration and transition from recognizable Dunmere infrastructure into a much older layer.

Hard rule:
> **The waterworks remain genuinely covert.**

No routine Black Host patrols. Plausible creatures/hazards/non-Host threats only. Architecture progresses from Dunmere/Yahtrean construction into older reused foundations and then clearly ancient construction.

### Sealed future-Hunt branch
A short way into the Old Waterworks, the party encounters a **real side-branch door that cannot be opened during Chapter 2**.

Locks:
- the door is visible and memorable, not hidden;
- Torren may inspect it, but the current party has no usable way to open it;
- it is **not** a Chapter-2 key puzzle, switch hunt, breakable wall, or optional detour that can be solved early;
- no Chapter-2 dialogue identifies what lies beyond it;
- no tutorial text needs to announce “come back later”;
- the door remains a persistent return point and will later lead into a Hunt route under Hunt authority.

Water is an ordinary environmental pressure throughout the works. The route may force **Cyanis, Ilyra, Torren, and Maevra all to get thoroughly wet**. Do not single out Torren's belongings or pre-signpost the later `Still Burns` joke here.

End → Sunken Archive / Beat 5.

---

## Beat 5 — Sunken Archive Entrance
Purpose: establish the ancient complex as a substantial location.

- **Sunken Archive** is the production/gameplay name; characters need not know a formal ancient name.
- major ancient scale;
- preservation/storage architecture;
- meaningful water intrusion;
- related to prior Diysean structures but functionally distinct;
- older than the waterworks and Old Bastion;
- not an active Black Host facility.

End → Beat 7.

---

## Legacy B06 — Western Map Mural — RETIRED
There is **no Chapter-2 map mural scene or second major cartographic discovery**.

The Chapter-1 Wayfinder remains the party's latest major Ancient map evidence throughout Chapter 2. Do not add a replacement wall map, route plate, Crest-marked map structure, Caelora-city map reveal, or equivalent cartographic payoff elsewhere in this chapter.

The B06 identifier is retired only to preserve stable downstream production/runtime numbering. Current story flow goes directly from Beat 5 to Beat 7.

---

## Beat 7 — Archive Depths
Purpose: substantial deeper exploration of the Ancient complex before the boss.

- worsening environmental pressure/hazards;
- no major cartographic/lore reveal;
- no routine Black Host presence;
- no clean alternate route;
- Archive Leviathan blocks progression.

End → Beat 8.

---

## Beat 8 — Archive Leviathan
Purpose: Sunken Archive combat climax.

- this is not the Major Hunt Worldscar Leviathan;
- combat party Cyanis + Ilyra + Torren;
- Maevra never fights;
- current one HP bar / two behavior-state structure unless encounter authority revises;
- exact mechanics/rewards remain encounter authority;
- victory opens physical progression, not exposition or records.

End → Beat 9.

---

## Beat 9 — Secret Passage Discovery
Purpose: connect the ancient Archive directly to the Old Bastion prisoner infrastructure.

- old concealed passage belongs to the ancient structure;
- not a Black Host tunnel;
- not known to the Dunmere elder;
- not the normal prisoner-transfer route;
- crosses into the Old Bastion's reused structure and emerges into/at the Prisoner Galleries.

Hard rule:
> **There is no transfer-records story beat between the Archive and the prisoners.**

End → Beat 10.

---

## Beat 10 — Prisoner Galleries / First Contact
Purpose: make the kidnapping operation human and reveal the Old Bastion's holding/transit function through lived prisoner experience.

Current prisoners include:
- travelers;
- injured soldiers;
- Greenhollow residents;
- Dunmere residents.

Prisoners know:
- people are brought to the Old Bastion and held;
- earlier captives were taken somewhere else and did not return;
- the Old Bastion functions as a **holding/transit site**, not the final destination.

They do not know the onward destination, ultimate fate, full strategic purpose, or exact transfer total.

Hard cooperation rule:
> **The prisoners want rescue and accept needed help.**

Ilyra asks consent before non-emergency examination/treatment where appropriate; refusal is not manufactured as conflict.

Prisoners credibly identify **Commander Rhazek as the officer commanding the Old Bastion**. The party recognizes him as a known high-ranking Black Host commander.

End → local part of galleries freed, but escape remains unsafe while Rhazek controls the fort / Beat 11.

---

## Beat 11 — Prisoner Safe Area / Exit Problem
Purpose: create a defensible lower safe area and convert the rescue into a tactical requirement.

- prisoners retain agency/control of their safe-area interior;
- secret passage → Archive → waterworks route is unsuitable for a mixed group containing injured/weakened people;
- practical exit is the maintained Old Bastion exit onto the Greenhollow–Dunmere road;
- active command layer controls it.

Decision:
**Cyanis, Ilyra, and Torren push upward → reach Rhazek → break local Host control → secure/open the proper exit → return for everyone.**

Maevra remains noncombat and may help organize the prisoners.

End → Beat 12.

---

## Beat 12 — Bastion Command Ascent
Purpose: transition into the active Black Host fort and carry the party toward Rhazek.

Hard rule:
> **The Old Bastion is a functioning Black Host military position, not an abandoned villain dungeon.**

Show a coherent subset of guard rotations, maintained equipment/supplies, logistical/support spaces, messenger/order traffic, gates/checkpoints, prisoner handling/staging, and legitimate care of Host wounded. Do not recreate the retired records beat through manifests, exact totals, or a destination ledger.

Resistance becomes organized as surprise erodes. No infinite reinforcements. The masked officer is reserved for Beat 13.

End → Beat 13.

---

## Beat 13 — Rhazek / Masked Officer
Purpose: direct confrontation, establish Rhazek's policy ownership, and seed Seyrik without identification.

Hard rule:
> **Rhazek is already working when the party arrives.**

An unnamed masked second officer is reporting to him. Writer-facing truth: **Seyrik Rell**. Player-facing firewall: unnamed, masked, not correctly identified, no subtitle/document/history/future-recruitment reveal, and no exaggerated future-party framing.

The masked officer leaves on credible operational business/order before combat and does not participate in Beat 14.

Rhazek knowingly authorized and commands the Old Bastion's kidnapping/forced-transfer policy. He does not deny it, blame rogue soldiers, or pretend coercion is consent. He does not reveal the onward destination.

End → Beat 14.

---

## Beat 14 — Commander Rhazek / Bastion Master
Purpose: command-layer combat climax.

Locks:
- Cyanis + Ilyra + Torren fight;
- Maevra never fights;
- masked officer absent;
- one HP bar unless encounter authority revises;
- finite prepared Host support allowed;
- story victory means Rhazek can no longer retain local command/control, not that every Host soldier dies.

Hard victory rule:
> **The party wins the Old Bastion locally. Rhazek's survival does not erase that victory.**

Rhazek survives a credible military withdrawal. No teleport, magical vanish, secret extra phase, invulnerable reversal, or mandatory villain stinger. The party prioritizes the prisoners over pursuit.

Victory provides the practical access/control needed to make the maintained road-side exit usable. No post-boss switch dungeon.

End → Beat 15.

---

## Beat 15 — Return to the Prisoners / Release
Purpose: complete the Bastion-side rescue as a human action.

Starting state:
- Rhazek defeated locally and withdrawn alive;
- Old Bastion command layer broken;
- road-side exit secured/openable;
- prisoners still waiting below;
- earlier transferred captives unresolved;
- Maevra remains noncombat.

Hard rule:
> **The party comes back for them.**

The trio physically returns. Prisoners are told Rhazek has been defeated here, local Host control is broken, the proper exit is available, and they can leave. They are ready and willing to go.

- able-bodied prisoners help injured/weakened people;
- Ilyra handles immediate treatment/mobility needs;
- Maevra organizes support/movement;
- Torren confirms/leads the safe route;
- Cyanis prioritizes completing the rescue over chasing Rhazek.

Direct-transition rule:
> **There is no dedicated playable prisoner-escort beat and no separate road-secured beat.**

Once prisoners are freed, organized, and beginning to leave, the story may transition directly to Dunmere.

End → Beat 16.

---

## Beat 16 — Return to Dunmere / Road Reopened / Chapter-2 Cleanup
Purpose: confirm the rescue and world-state consequences economically and open the cleanup window.

Opening:
- Cyanis, Ilyra, Torren, and Maevra are back in Dunmere;
- the current rescued people are safely out of Black Host custody;
- no dedicated escort gameplay or second road-safety proof scene is required.

Mandatory Dunmere elder resolution:
- current prisoners were successfully freed;
- the Old Bastion no longer controls the corridor for the Black Host;
- direct **Greenhollow ↔ Dunmere travel is reopened and safe for ordinary travel**;
- earlier prisoners transferred away before the rescue remain missing and their destination remains unknown.

Knowledge provenance:
- the elder may know the local consequences: people are back/out safely, road movement is resuming, and the local choke point is broken;
- the party supplies protected inside-Bastion information such as Rhazek's personal command, his withdrawal alive, the holding/transit function, and earlier transferred groups.

Do not turn the scene into a victory ceremony, tactical debrief, immediate new quest briefing, or Ancient-map explanation.

Afterward:
- Dunmere is the Chapter-2 cleanup hub;
- compatible Character-Life scenes, side content, Hunts, shopping, healing/rest, equipment management, saving, permitted backtracking, and reopened-road travel may occur under their own authorities;
- Maevra remains physically present;
- the larger journey still points east toward Caelora;
- Chapter 3 does not start automatically.

Hard chapter-boundary rule:
> **Chapter 3 begins only when the player deliberately chooses the explicit advance-story interaction/confirmation.**

---

# Character-Life cleanup authority
- **C06 — Still Burns** is the current Chapter-2 cleanup Character-Life scene for Cyanis, Ilyra, Torren, and Maevra.
- Former separate C07 is retired; its slot/function is folded into C06.

# Reveal firewall
Do not reveal in Chapter 2:
- destination or ultimate fate of previously transferred captives;
- complete purpose of the wider Black Host transfer operation;
- exact transfer total / retired `thirty-one transfers` clue;
- Seyrik's identity/history/future role;
- complete Prime/Last Sentinel answers;
- full Diysean route/network explanation;
- any Chapter-2 replacement reveal that fills in the Wayfinder's broken Caelora-area center;
- any Crest-marked cartographic structure or equivalent replacement for the retired mural;
- Cresthaven;
- buried-Crest truth;
- Entity.
# Chapter 1 — The Junction / Hidden Monument Lock

**Status:** CURRENT EXPLICIT USER DIRECTION  
**Domain:** Chapter 1 story / Torren knowledge / mural-to-Junction continuity / expanded Wayfinder map scope

## Hard correction

The location previously spoken of in Chapter-1 dialogue as `Wayfinder` is called **the Junction** by Torren and the party at this stage.

Torren knows the place as an **ancient crossroads** on the modern southern route network. He has been through / around the crossroads before and knows where it is, but he has **never seen the actual ancient cartographic monument hidden there**.

## Hollow Watch mural

The protected Hollow Watch mural must show:
- recognizable regional geography;
- Hollow Watch's own hill;
- the ancient crossroads farther south that Torren recognizes as **the Junction**;
- a **large, clearly important monument positioned at those crossroads**;
- the monument is visually distinct from the multiple deep tunnel entrances shown elsewhere in the mural;
- no crowd is shown descending beneath the monument and it is not presented as another tunnel entrance.

Torren recognizes the **location / crossroads**, not the monument itself.

He must not say he has seen the monument before, must not recognize its exact shape from firsthand experience, and must not call it `Wayfinder` in Chapter-1 dialogue before the later discovery establishes what the hidden stone actually is.

## Arrival at the Junction

When the party later reaches the Junction:
- the ancient crossroads are familiar to Torren as a place / route feature;
- the important monument seen in the mural is not immediately obvious in the present-day landscape;
- its stone surface is heavily covered by **vines and moss**;
- the party physically pulls / clears the vegetation away;
- only then do they reveal the preserved **map / cartographic surface** on the monument;
- this is the first time Torren actually sees the monument's map surface.

The reveal should feel like the mural told them **something important exists at the crossroads**, while the present-day vegetation explains why Torren and ordinary travelers never realized what was there.

## Expanded Wayfinder map lock

Once the vegetation is cleared, the cartographic surface is **not** merely a local Junction route plate.

It depicts a substantial **western portion of ancient Diyse**:
- dense overland routes intersect and reconnect throughout the mapped region;
- a visually distinct second system of markings corresponds in underlying map authority to extensive underground infrastructure and routes, but is **not legible to the Chapter-1 party as underground**;
- built-up city / settlement structures appear in many locations;
- the largest visible concentration of urban structures, route convergence, and underground infrastructure lies around the geography corresponding to **present-day Caelora**;
- the Wayfinder is physically broken through that Caelora-area concentration, so the center of the largest ancient cluster is missing;
- surviving lines visibly converge toward and continue around the broken area, making the scale of what is missing obvious without revealing the lost center itself.

The party may recognize that the map represents a much larger and more interconnected ancient landscape than the modern western region. They can identify the obvious overland routes and can see that a second extensive system of markings exists, but they **cannot identify what that second system represents yet**. In Chapter 1 they do not call it underground, tunnels, conduits, service routes, or subsurface infrastructure. They also do not know the identities/functions of the many ancient urban sites or what specifically occupied the missing Caelora-area center.

The local information remains usable:
- the surviving Dunmere-bound alignment matches the party's already-planned route;
- the water/cistern notation still corresponds to the earlier overgrown Southern Briar side access;
- Face markings remain part of the monument's technical grammar without their map function being explained.

The party's practical copy should preserve as much of the surviving western map as possible, including the broad route relationships, the unexplained second marking system, city clusters, the shape/location of the Caelora-area break, and the local useful notations. They must not draw in or reconstruct the missing center.

## Knowledge rule

Torren's valid Chapter-1 knowledge is:
- where the Junction is;
- that it is an old / ancient crossroads;
- how the modern route passes through or near it;
- local road and terrain conditions.

Torren does **not** know before the vegetation is cleared:
- the hidden monument's map surface;
- its ancient cartographic function;
- its technical grammar;
- why the mural emphasizes it;
- how it relates to the wider Diysean route system.

After the reveal, Torren can read practical geographic relationships and recognize the obvious overland routes. He can tell the second marking system is distinct from those roads, but he **cannot identify it as underground infrastructure or explain what it represents**. He also cannot identify what is missing at the broken Caelora-area center.

## Naming

In current Chapter-1 character dialogue, use **the Junction** for the place.

`Wayfinder` may remain an internal / legacy production identifier where changing filenames or IDs would be disruptive, but it must not imply that Torren or the party already know the hidden monument by that name or have previously seen its exposed cartographic stone.

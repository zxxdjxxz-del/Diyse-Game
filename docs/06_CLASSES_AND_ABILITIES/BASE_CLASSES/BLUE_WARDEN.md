# Diyse — Blue Warden
**Migration baseline:** `Diyse_CURRENT_WORKING_TRACKER_CONSOLIDATED_2026-08-27_v85.md`  
**Master-canon class/resource authority:** **v2.08 / Audit123**, with compatible **Audit115** class normalization and later current working corrections.  
**Authority treatment:** explicit/newer working corrections are preserved as working overrides when they have not yet been promoted into the audit chain.


**Owner:** Ilyra  
**Class line:** Native Base Class  
**Identity:** healer/protector; Magic-based healing with balanced defenses  
**Trait:** **Gentle Continuance**  
**Ultimate:** **Dawn Without End**

## Ability spine
| Unlock | Ability | MP | Current effect |
|---:|---|---:|---|
| CL1 | **Mend** | 16 | **Power: N/A — no direct damage.** Direct healing: 22% target Max HP + 1.50 × Ilyra's Magic. |
| CL1 | **Clear Warding** | 18 | **Power: N/A — no direct damage.** Remove 1 eligible harmful status; grant **+5 Status Resistance for 2 rounds**. |
| CL1 | **Renewal** | 26 | **Power: N/A — no direct damage.** Party healing; does not automatically grant Regen. |
| CL3 | **Warden's Valor** | 18 | One enemy; Magical / Colorless; **170 Power**; no healing, status, or defensive rider. |
| CL6 | **Revive** | 34 | **Power: N/A — no direct damage.** Revive one KO ally at 35% Max HP. |
| CL9 | **Lifeline** | 40 | **Power: N/A — no direct damage.** Prepared KO-prevention: target remains at 1 HP, then recovers 18% Max HP + 0.80 × Ilyra's Magic. |
| CL13 | **Dawn Without End** | 60 | Base Ultimate; all enemies; Magical / Colorless; **220 Power per target** plus current revive/heal/cleanse support and 5% Max-HP Regen × 3 rounds. |

## Masteries
| Unlock | Mastery | Current effect |
|---:|---|---|
| CL3 | **Gentle Hands** | Mend gains +10% healing potency. |
| CL6 | **Clear Heart** | Clear Warding additionally removes 1 eligible ordinary stat reduction. |
| CL9 | **Restored Breath** | Revive recovery 35% → 45% target Max HP. |
| CL12 | **Stronger Lifeline** | Lifeline emergency Max-HP heal component 18% → 23%; +0.80 × Magic remains unchanged. |

All four Core Masteries unlock automatically at the listed Class Levels under the current v85 working override.


## Global references
- Damage/penetration: `05_BATTLE_SYSTEM/DAMAGE_FORMULAS.md`
- Hit/Evasion: `05_BATTLE_SYSTEM/BASE_HIT_AND_EVASION.md`
- Critical: `05_BATTLE_SYSTEM/CRITICAL_HITS.md`
- Elements/statuses: `05_BATTLE_SYSTEM/ELEMENTS.md` and `STATUS_EFFECTS.md`

## Firewall
Do not restore removed historical mechanics merely because an archived version of this class used them.

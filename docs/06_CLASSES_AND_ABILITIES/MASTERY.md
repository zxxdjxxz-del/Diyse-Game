# Diyse — Current Mastery Architecture
**Migration baseline:** `Diyse_CURRENT_WORKING_TRACKER_CONSOLIDATED_2026-08-27_v85.md`  
**Master-canon class/resource authority:** **v2.08 / Audit123**, with compatible **Audit115** class normalization and later current working corrections.  
**Authority treatment:** explicit/newer working corrections are preserved as working overrides when they have not yet been promoted into the audit chain.

# Current active working rule
**Mastery Points do not exist in the active v85 working branch.**

Masteries unlock automatically by Class Level.

This supersedes the old tracker-level:
- 8-point schedule;
- point banking;
- point spending;
- Sixfold Volition Mastery Point;
- Lv40 / Lv50 / Lv60 Mastery Point grants.

It has **not yet been formally promoted into the Audit123 master-canon file**, so preserve that authority distinction in historical documentation.

## Core Masteries
| Character | Class | Unlock | Mastery | Effect |
|---|---|---:|---|---|
| Cyanis | Crest Knight | CL3 | **Guardian Geometry** | When Crest Reprisal successfully intercepts an eligible single-target direct attack, Cyanis takes 10% less final direct damage from that intercepted attack. |
| Cyanis | Crest Knight | CL6 | **Balanced Assault** | Crest Strike +15 Power; Twin Advance +15 total Power, not +15 per hit. |
| Cyanis | Crest Knight | CL9 | **Clear Channel** | Resonant Pulse costs 1 less MP: 12 → 11. |
| Cyanis | Crest Knight | CL12 | **Penetrating Crest** | Crest Rend gains +10 percentage points of applicable penetration on both its Attack-derived and Magic-derived shares. |
| Ilyra | Blue Warden | CL3 | **Gentle Hands** | Mend gains +10% healing potency. |
| Ilyra | Blue Warden | CL6 | **Clear Heart** | Clear Warding additionally removes 1 eligible ordinary stat reduction. |
| Ilyra | Blue Warden | CL9 | **Restored Breath** | Revive recovery 35% → 45% target Max HP. |
| Ilyra | Blue Warden | CL12 | **Stronger Lifeline** | Lifeline emergency Max-HP heal component 18% → 23%; +0.80 × Magic remains unchanged. |
| Torren | War Archer | CL3 | **Veteran's Eye** | Sizing Shot **135 → 155 Power**. |
| Torren | War Archer | CL6 | **Heavy Draw** | Colossus Draw **245 → 265 Power**. |
| Torren | War Archer | CL9 | **Fieldcraft** | Choose the Route improves Clear Route **+10 → +15 Base Hit** and Covered Route **+10 → +15 Evasion**. |
| Torren | War Archer | CL12 | **Relentless Hand** | Relentless Barrage AoE portion **160 → 180 Power per target**; measured-target follow-up remains 80 Power. |
| Nimera | Cardweaver | CL3 | **Quick Study** | Echo Weave copy potency **80% → 90%** for direct damage, direct healing, and revival recovery. |
| Nimera | Cardweaver | CL6 | **Charged Weave** | Weave Spark Stun **35% → 50%**. |
| Nimera | Cardweaver | CL9 | **Efficient Override** | Ancient Override party MP reduction **15% → 20%**. |
| Nimera | Cardweaver | CL12 | **Perfect Reconstruction** | Perfect Recall duration **3 → 4 rounds** and Grand Reweaving copy potency **75% → 85%** for direct damage, direct healing, and revival recovery. |
| Vaelira | Green Arcanist | CL3 | **Elemental Memory** | Prismatic Flow qualifying final-damage bonus +10% → +15%. |
| Vaelira | Green Arcanist | CL6 | **Elemental Fluency** | The four early fixed matching-status Abilities increase listed base status chance 20% → 25%; Vaelira's separate +5pp specialist bonus still applies afterward. |
| Vaelira | Green Arcanist | CL9 | **Efficient Spectrum** | First eligible elemental Green Arcanist Base Ability each ordinary round costs 2 less MP, minimum 1. |
| Vaelira | Green Arcanist | CL12 | **Spectrum Pressure** | Prism Lance +15 Power; Spectrum Cascade +15 Power. |
| Seyrik | Ruin Vanguard | CL3 | **Sharper Ruin** | Ruin Cleave +15 Power; Rift Lance +15 Power. |
| Seyrik | Ruin Vanguard | CL6 | **Branding Force** | Ember Brand Burn 20% → 30%; Fracturing Brand Bleed 20% → 30% base. |
| Seyrik | Ruin Vanguard | CL9 | **Bound Fang** | Shardfang Pounce 135 → 145 Power; Rend 100 → 110 Power. |
| Seyrik | Ruin Vanguard | CL12 | **Controlled Unmaking** | Unmaking Blow costs 2 less MP: 30 → 28; its Bleeding-target final-damage bonus +15% → +20%. |

## Subclass Masteries
| Character | Class | Unlock | Mastery | Effect |
|---|---|---:|---|---|
| Cyanis | Crest Arcanist | CL3 | **Arcane Force** | Arcane Lance 220 → 240 Power. |
| Cyanis | Crest Arcanist | CL5 | **Elemental Precision** | Elemental Crest 180 → 200 Power and +10 Base Hit. |
| Cyanis | Crest Arcanist | CL7 | **Equipment Mastery** | Donor Green Arcanist Relic access eligibility. |
| Cyanis | Crest Arcanist | CL11 | **Legacy Mastery** | Donor Green Arcanist Legacy access eligibility. |
| Ilyra | Vowblade | CL3 | **Tempered Vow** | Vital Edge 185 → 205 Power. |
| Ilyra | Vowblade | CL5 | **Life Through Steel** | Mercy Returned other-ally Max-HP healing component 12% → 15%; +0.90 × Magic unchanged. |
| Ilyra | Vowblade | CL7 | **Equipment Mastery** | Donor Ruin Vanguard Relic access eligibility. |
| Ilyra | Vowblade | CL11 | **Legacy Mastery** | Donor Ruin Vanguard Legacy access eligibility. |
| Torren | Routeweaver | CL3 | **Long Sight** | Throughline's post-application full-round window increases by **1 full normal round**. |
| Torren | Routeweaver | CL5 | **Guided Crossing** | Set the Pace **Speed +20% → +30%**; a Covered Crossing ally also gains **+10 Evasion** from Torren's turn completion until the routed action finishes resolving. |
| Torren | Routeweaver | CL7 | **Equipment Mastery** | Donor Cardweaver Relic access eligibility. |
| Torren | Routeweaver | CL11 | **Legacy Mastery** | Donor Cardweaver Legacy access eligibility. |
| Nimera | Proofhunter | CL3 | **Proven Measure** | Measured Shot **140 → 155 Power**; its already-Measured Critical bonus **+15pp → +20pp**. |
| Nimera | Proofhunter | CL5 | **Decisive Argument** | Held Argument **175 → 190 Power**; against Hunter's Measure its empowered Power becomes **225** instead of 210. |
| Nimera | Proofhunter | CL7 | **Equipment Mastery** | Donor War Archer Relic access eligibility. |
| Nimera | Proofhunter | CL11 | **Legacy Mastery** | Donor War Archer Legacy access eligibility. |
| Vaelira | Axiomblade | CL3 | **First Corollary** | First Principle 150 → 170 Power. |
| Vaelira | Axiomblade | CL5 | **Elemental Proof** | Proven Advance Burn 20% → 25%; Counterproof Stun 20% → 25%; Vaelira's separate specialist bonus still applies where legal. |
| Vaelira | Axiomblade | CL7 | **Equipment Mastery** | Donor Crest Knight Relic access eligibility. |
| Vaelira | Axiomblade | CL11 | **Legacy Mastery** | Donor Crest Knight Legacy access eligibility. |
| Seyrik | Ruin Warden | CL3 | **Deeper Siphon** | Siphon Rune recovery 35% → 40% of eligible damage; 20% Max-HP cap unchanged. |
| Seyrik | Ruin Warden | CL5 | **Shared Grace** | Stolen Grace Magic coefficient 0.35 → 0.45; 25% target-Max-HP cap unchanged. |
| Seyrik | Ruin Warden | CL7 | **Equipment Mastery** | Donor Blue Warden Relic access eligibility. |
| Seyrik | Ruin Warden | CL11 | **Legacy Mastery** | Donor Blue Warden Legacy access eligibility. |

## Donor map

| Character | Subclass | Donor Base tradition |
|---|---|---|
| Cyanis | Crest Arcanist | Green Arcanist |
| Ilyra | Vowblade | Ruin Vanguard |
| Torren | Routeweaver | Cardweaver |
| Nimera | Proofhunter | War Archer |
| Vaelira | Axiomblade | Crest Knight |
| Seyrik | Ruin Warden | Blue Warden |

## Native Legacy firewall

A character's own native Base-Class Legacy:
- does not require Legacy Mastery;
- does not require Synthesis;
- does not require a Mastery Point.

## Historical mechanical Node-3 quarantine

Before the v85 restructuring, each Subclass had a third mechanical Mastery:
- Crest Arcanist — Perfect Nullification
- Vowblade — Unfaltering Edge
- Routeweaver — Safe Passage
- Proofhunter — Verified Harm
- Axiomblade — Exact Equivalence
- Ruin Warden — Strengthened Ward

Those effects are **not active Subclass Mastery nodes in the current v85 four-node structure** unless a later explicit rule rehomes them. Do not silently preserve them beside Equipment/Legacy Mastery and accidentally create five or six Subclass Masteries.

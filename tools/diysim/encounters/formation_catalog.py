...
def _singularize_last_word(value: str) -> str:
    ...
    if word.endswith("men") and len(word) > 3:
        ...
        # echoes/heroes style plurals -> echo/hero
        word = word[:-2]
...
def _enemy_index(
    ...
    """Build exact authority aliases plus conservative unique-title suffix candidates.

    File stems are owner identifiers and are safe aliases even when an owner title
    carries an authority/status suffix. Shortened title matching is deliberately
    not resolved here unless exactly one owner can satisfy it.
    """
    ...
    for definition in definitions:
        ...
        # Two-or-more-word suffixes allow formation shorthand such as
        # `Sky Skirmisher` -> `Black Host Sky Skirmisher` without accepting a
        # dangerously broad one-word alias like `Stalker`.
...
def _parse_member(
    ...
    definition = _resolve_enemy(label, index, suffix_candidates)
    ...
    *,
    ...
    for table in extract_markdown_tables(text):
        ...
        for row in table:
            ...
            members = tuple(
                _parse_member(part, index, suffix_candidates)

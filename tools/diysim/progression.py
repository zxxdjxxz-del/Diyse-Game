...
def exp_at_level_progress(level: int, progress: float = 0.0) -> int:
    """Return cumulative EXP at a fraction through the requested level.

    `progress=0.0` means exactly at the level threshold. `progress=0.5` means
    halfway from that level to the next. At Lv70 the only legal progress is 0.
    """
...
def required_average_exp_per_encounter(
    *,
    ...
    """Solve the average encounter EXP needed to reach a target checkpoint.

    This is a planning equation only. It does not silently rewrite authored
    encounter rewards; it tells the designer what average reward budget would
    be required for the chosen route assumptions.
    """

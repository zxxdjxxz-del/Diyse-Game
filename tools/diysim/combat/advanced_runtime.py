"""Advanced battle runtime using sectioned current-canon combat primitives."""
...
from .models import BASIC_ATTACK, CombatAction, CombatUnit, Combatant, StatusRider
...
def _targets(actor: CombatUnit, action: CombatAction, units: Sequence[CombatUnit], rng: random.Random) -> list[CombatUnit]:
    if action.target_side == "enemy":
        candidates = [u for u in _enemies(actor, units) if u.alive]
    elif action.target_side == "self":
        candidates = [actor] if actor.alive else []
    else:
        candidates = [u for u in _allies(actor, units) if u.alive]
    if action.action_kind == "heal":
...
def resolve_heal(actor: CombatUnit, action: CombatAction, target: CombatUnit) -> int:
    ...
    if action.clear_harmful_statuses:
        _clear_statuses(target, action.clear_harmful_statuses)
...
def resolve_damage(actor: CombatUnit, action: CombatAction, target: CombatUnit, rng: random.Random) -> int:
    ...
    if target.alive:
        for rider in action.status_riders:
            _roll_status(action, rider, target, rng)
    return damage
...
def _outcome(winner, rounds, party, any_ko) -> AdvancedBattleOutcome:
    ...
    return AdvancedBattleOutcome(
        ...
        rounds,
        any_ko,
        sum(u.hp for u in party) / hp_total if hp_total else 0.0,
        sum(u.mp for u in party) / mp_total if mp_total else 1.0,
    )
    ...
    *,
    ...
    for round_number in range(1, max_rounds + 1):
        for actor in turn_order(units):
            if not actor.alive:
                continue
            if not any(u.alive for u in (enemies if actor.side == "party" else party)):
                break
        ...
        if not enemy_alive:
            return _outcome("party", round_number, party, any_ko)
        if not party_alive:
            return _outcome("enemy", round_number, party, True)

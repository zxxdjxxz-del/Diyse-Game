...
### `combat/`
Owns combat-system mechanics only. Subsystems should remain individually identifiable: damage, hit/evasion, criticals, elements, statuses, MP, healing, turn/round sequencing, targeting, Guard/Defend, prepared actions, Fields, Items, Cards, and Primes.
...
Mandatory/story boss bodies, actions, phases/forms, AI, and boss-local support references.

### `content/regional_hunts/`
Regional Hunt definitions.

### `content/major_hunts/`
Major Hunt definitions.
...
### `scenarios/`
Battle setups: party snapshot, enemy formation, story point, starting HP/MP, legal inventory/loadout, policy, seed/run defaults, and expected regression ranges. Scenario files compose reusable definitions; they do not own canonical actor stats.
...
The existing root modules (`core.py`, `rules.py`, `battle.py`, `progression.py`, `io.py`) remain temporarily as compatibility entry points while code is migrated section-by-section. New work should be routed into the section that owns it instead of enlarging those files indefinitely.
